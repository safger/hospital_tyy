...
<body class="skin-blue layout-top-nav">
						...
						<div class="box-body">
							<div class="head-find">
								...
								<div class="row">
									<div class="col-xs-3">
										<div class="form-new-input">
											<span>置管护士：</span> <input type="text" id="nursingNurse" class="fis">
										</div>
									</div> 
									<div class="col-xs-3">  
										<div class="form-new-input">
											<span>穿刺原因：</span> <select id="punctureCauses" class="fis">
								...
								<div class="row">
									<div class="col-xs-3">
										<div class="form-new-input">
											<span>穿刺方式：</span> <select id="punctureMethod"  class="fis">
												<option value="-1">所有</option>
												<option>B超引导</option>
												<option>盲穿</option>
												</select>
									...
									<div class="col-xs-3">  
										<div class="form-new-input">
											<span>穿刺部位：</span> <select id="punctureSite"  class="fis">
												<option value="-1">所有</option>
												<option>右臂肘上</option>
												<option>右臂肘下</option>
												<option>右臂肘正中</option>
									...
									<div class="col-xs-6">  
										<div class="form-new-input">
											<span>穿刺静脉：</span><select id="punctureVein" class="fis">
														<option value="-1">所有</option>
														<option>贵要静脉</option>
														<option>肘正中静脉</option>
														<option>头静脉</option>
					...
					<div class="col-xs-12">
						<div class="box">
							<div class="box-header">
								<h3 class="box-title">PICC穿刺查询</h3>
	...
	<form id="mform" action="<%=basePath%>backstage/piccPunctureregister/showEdit.html" method="post">
      	...
      	<input type="hidden" id="punctureId" name="punctureId" value="" /> 
      </form>
	<!-- Modal detail -->
...
$(function() {
    ...
    var template = Handlebars.compile(tpl);
	 ...
	 table = $('#newAttributeTable').dataTable( { 
        ...
        "ajax": "<%=basePath%>backstage/piccPunctureregister/getDate.html",
        "columns": [{ "data": null, 
					 ...
					 { 
					  ...
					  },  
					  {
	                        ...
	                        render: function (data, type, row,   meta) {
	                            ...
	                            {
	                            		 func: [
	   	                                    {"name": "修改","fn": "edit(\'" + row.patientId + "\',\'" + row.fuid + "\')", "type": "primary"},
    ...
    function del(fuid) {
        ...
        $.ajax({
            url: "<%=basePath%>backstage/piccPunctureregister/delete.html",
            data: {
                "fuid": fuid
    ...
    function gsearch() {
		var punctureCauses = $("#punctureCauses option:selected").val();
		var ward = $("#ward").val();
		var clinicnumber = $("#clinicnumber").val();
		var patient = $("#patient").val();
		var nursingNurse = $("#nursingNurse").val();
		var stime = $("#sstime").val() + " ";
		var etime = $("#setime").val() + " ";
		var hospitalnumber=$("#hospitalnumber").val();
		var punctureMethod= $("#punctureMethod option:selected").val();
		var punctureSite=$("#punctureSite option:selected").val();
		var punctureVein=$("#punctureVein option:selected").val();
		var data = punctureCauses + "," + ward + "," + clinicnumber + ","
				+ patient + "," + stime + "," + etime + ","
				+ nursingNurse+","+hospitalnumber+","+punctureMethod+","+punctureSite+","+punctureVein;
		table.fnFilter(data);
	} 
    //时间格式化 
